...
export default function TodayPage() {
  ...
  const readings = useMemo(
    () => readFixtures(DEMO_FIXTURES, settings),
    [settings]
  );
    ...
    <main className="animate-rise space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-ink-muted">
          LaLiga · {DEMO_FIXTURES.length} partidos analizados
        </p>
      </header>
          ...
          <section className="space-y-3">
            ...
            {bets.length ? (
              ...
              <Card>
                <CardContent>
                  <p className="text-sm text-ink-muted leading-relaxed">
                    Hoy no hay ningún mercado que supere tu umbral de{" "}
                    {settings.minEvPct}% de EV. No apostar también es una
                    decisión: forzar apuestas sin valor es la vía más rápida a
                    perder el banco.
                  </p>
                </CardContent>
              </Card>
                ...
                <CardContent className="flex items-center justify-between gap-3 p-4">
                  <div className="min-w-0 space-y-1">
                    ...
                    <p className="text-xs text-ink-faint">
                      {formatKickoff(fixture.kickoff)} · margen{" "}
      ...
      <p className="text-xs text-ink-faint leading-relaxed">
        Datos de demostración hasta conectar proveedores reales. BetLocal informa
        sobre desviaciones de precio; no predice resultados ni garantiza
        ganancias. +18.
      </p>
    </main>
  );
